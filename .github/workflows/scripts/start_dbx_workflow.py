...
parser.add_argument("--host", type=str, required=True, help="Databricks host.")
parser.add_argument(
    "--workflow-names",
    nargs="*",
    type=str,
...
def main():
    ...
    for workflow_name in workflows:
        ...
        job_id = get_job_id(workflow_name, host=args.host, http_header=http_header)

        # Start the job when found
        if job_id:
            print(f"Starting workflow '{workflow_name}' with id '{job_id}'.")
            start_job(job_id=job_id, queue=True, host=args.host, http_header=http_header)
            started_workflows += 1
